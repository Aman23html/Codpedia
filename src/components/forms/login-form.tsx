...
// Assuming this is your actual server action path
    ...
    startTransition(async () => {
      try {
        ...
        if (result && result.success === false) {
          setMessage(result.message || "Authentication failed. Please verify your credentials.");
          return;
        }

        // Trigger optimistic success UI
        setIsSuccess(true);
        setMessage("Authentication verified. Routing to dashboard...");

        // Delay redirect slightly to allow the success animation to play
        setTimeout(() => {
        ...
        setMessage("A secure connection could not be established. Please try again.");
      }
    });
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-[var(--background)] font-sans flex flex-col text-[var(--foreground)] selection:bg-[var(--primary)] selection:text-white">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto w-full px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 min-h-screen overflow-x-hidden pt-20 lg:pt-32">
        ...
        {/* ========================================== */}
        {/* LEFT COLUMN: BRANDING & SYSTEM STATUS      */}
        {/* ========================================== */}
        <div className="lg:col-span-7 flex flex-col pt-8 lg:pt-16 pb-16 lg:pr-24">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex-1">
            ...
            <motion.p variants={fadeUp} className="text-lg text-[var(--muted-foreground)] font-medium max-w-lg mb-12 leading-relaxed">
              Execution turns plans into results. Access your <strong className="text-[var(--foreground)]">Codepedia EMS</strong> dashboard to monitor operations, manage tasks, and drive global impact.
            </motion.p>
          </motion.div>

          {/* System Status Footer (Inspired by reference image) */}
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="mt-auto grid grid-cols-1 md:grid-cols-2 gap-8 p-6 rounded-[24px] bg-[var(--card)]/30 border border-[var(--border)] backdrop-blur-xl">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[11px] font-bold text-[var(--foreground)] uppercase tracking-widest">System Status: Operational</span>
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-[var(--muted-foreground)]"><CheckCircle2 className="w-4 h-4 text-[#2ECC71]" /> Database Online</div>
                <div className="flex items-center gap-2 text-xs text-[var(--muted-foreground)]"><CheckCircle2 className="w-4 h-4 text-[#2ECC71]" /> Task Engine Running</div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[11px] font-bold text-[var(--foreground)] uppercase tracking-widest">Secure Environment</span>
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-[var(--muted-foreground)]"><CheckCircle2 className="w-4 h-4 text-[var(--primary)]" /> Role-Based Access Control</div>
                <div className="flex items-center gap-2 text-xs text-[var(--muted-foreground)]"><CheckCircle2 className="w-4 h-4 text-[var(--primary)]" /> Encrypted Authentication</div>
              </div>
            </div>
            ...
            <div className="relative rounded-[2.5rem] bg-[var(--card)]/60 backdrop-blur-3xl border border-white/20 dark:border-white/10 shadow-2xl p-8 lg:p-10 z-10 overflow-hidden min-h-[500px] flex flex-col">
              ...
              <AnimatePresence mode="wait">
                ...
                {!isSuccess ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 flex-1 flex flex-col">
                      ...
                      <div className={`space-y-5 transition-all duration-300 ${isPending ? 'opacity-50 grayscale pointer-events-none' : 'opacity-100'}`}>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-[var(--foreground)]/70 uppercase tracking-widest ml-1">Employee ID / Email</label>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between items-center ml-1">
                            <label className="text-[10px] font-bold text-[var(--foreground)]/70 uppercase tracking-widest">Password</label>
                            <a href="#" className="text-[10px] font-bold text-[var(--primary)] hover:underline">Forgot Password?</a>
                          </div>
                          <div className="relative group/input">
                        ...
                        <div className="flex items-center gap-2 ml-1">
                          <input type="checkbox" id="remember" className="rounded bg-[var(--background)] border-[var(--border)] text-[var(--primary)] focus:ring-[var(--primary)]/20" />
                          <label htmlFor="remember" className="text-xs text-[var(--muted-foreground)] font-medium cursor-pointer">Keep me securely logged in</label>
                        </div>
                      </div>
                      ...
                      <div className="pt-4 mt-auto relative z-20">
                        ...
                        <div className="mt-8 flex items-center gap-4">
                          ...
                          <span className="text-xs font-bold text-[var(--muted-foreground)] uppercase tracking-widest">Access Required?</span>
                          <div className="h-px bg-[var(--border)] flex-1" />
                        </div>
                        
                        <div className="flex justify-center items-center text-xs font-bold pt-6 opacity-70 hover:opacity-100 transition-opacity">
                          <a href="/register" className="flex items-center gap-2 text-[var(--foreground)] hover:text-[var(--primary)] transition-colors">
                            <Server className="w-4 h-4" /> Request Workspace Provisioning
                          </a>
                        </div>
                      </div>
                    </form>
                  ...
                  /* -------------------------------------- */
                  /* STATE 2: THE SUCCESS ROUTING SCREEN    */
                  /* -------------------------------------- */
                  <motion.div 
                    key="success-routing"
                  ...
                  >
                    ...
                    <motion.div 
                      initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "200px" }} transition={{ delay: 0.8, duration: 1.5, ease: "linear" }}
