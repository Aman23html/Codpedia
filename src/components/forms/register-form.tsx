...
export default function RegisterForm({ departments }: RegisterFormProps) {
  ...
  const getFriendlyErrorMessage = (message?: string) => {
    ...
    if (msg.includes("phone")) {
      return "This phone number is already registered. Please use another phone number.";
    }

    if (msg.includes("department")) {
      return "Please select a valid department before registering.";
    }

    if (msg.includes("password")) {
      return "Password is not valid. Please enter a stronger password.";
    }

    if (msg.includes("required")) {
      return "Please fill all required fields before registering.";
    }
    ...
    if (msg.includes("server")) {
      return "Server error occurred while creating your account. Please try again.";
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (password !== confirmPassword) {
      setErrorMsg("Passwords do not match. Please enter the same password again.");
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        if (result?.success) {
          ...
          setErrorMsg(getFriendlyErrorMessage(result?.message));
        }
      } catch (error) {
        console.error("Registration error:", error);

        setErrorMsg(
          "Registration could not be completed due to a server error. Please try again or contact the administrator."
        );
      }
    });
  };
                    ...
                    >
                      ...
                      {errorMsg && (
                        <motion.div
                          ...
                          className="flex items-start gap-3 p-4 rounded-2xl border text-sm font-semibold shadow-sm bg-red-500/10 border-red-500/30 text-red-500"
                        >
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="relative group/input">
                              ...
                              <input
                                ...
                                className="w-full rounded-2xl bg-[var(--background)]/50 border border-[var(--border)] pl-11 pr-4 py-3.5 text-sm outline-none shadow-inner focus:border-[var(--primary)] focus:ring-4 focus:ring-[var(--primary)]/10 transition-all"
                              />
                            </div>
                          </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="relative group/input">
                              ...
                              <select
                                ...
                                onChange={handleChange}
                                className="w-full rounded-2xl bg-[var(--background)]/50 border border-[var(--border)] pl-11 pr-10 py-3.5 text-sm outline-none shadow-inner focus:border-[var(--primary)] focus:ring-4 focus:ring-[var(--primary)]/10 transition-all cursor-pointer appearance-none"
                              >
                              ...
                              <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                                ...
                                >
                                  <path
                                    ...
                                    className="text-[var(--muted-foreground)] group-focus-within/input:text-[var(--primary)] transition-colors"
                                  />
                                </svg>
                              </div>
                            </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="relative group/input">
                              ...
                              <input
                                ...
                                onChange={handleChange}
                                className="w-full rounded-2xl bg-[var(--background)]/50 border border-[var(--border)] pl-11 pr-12 py-3.5 text-sm outline-none shadow-inner focus:border-[var(--primary)] focus:ring-4 focus:ring-[var(--primary)]/10 transition-all"
                              />
                      ...
                      <div className="pt-6 relative z-20">
                        ...
                        <div className="flex justify-center items-center text-xs font-bold pt-6 opacity-70 hover:opacity-100 transition-opacity">
                          ...
                          >
                            ...
                            Existing Identity? Authenticate Here
                          </a>
                        </div>
                      </div>
                    </form>
