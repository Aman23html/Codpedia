          ...
          <tbody className="divide-y divide-[var(--border)]/60">
            {reports.length === 0 ? (
              ...
              reports.map((report) => {
                ...
                const userId = String(report.user?.id || report.user?._id || "");
                const employeeCode =
                  report.user?.employeeCode ||
                  (userId ? `EMP-${userId.substring(0, 6).toUpperCase()}` : "-");
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell mono>{report.queryGenerated ?? 0}</TableCell>

                    <TableCell mono>{report.dealsDone ?? 0}</TableCell>

                    <TableCell mono>
                      ₹{Number(report.dealsDoneAmount || 0).toLocaleString("en-IN")}
                    </TableCell>

                    <TableCell mono>{report.tutorAssigned ?? 0}</TableCell>
                    ...
                    <TableCell>
                      <Link
                        href={`/incharge/operations/reports/${reportId}`}
                        className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2 text-xs font-black text-white"
                      >
                        View
                        <ArrowRight className="h-3.5 w-3.5" />
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    DRAFT: "bg-slate-500/10 text-slate-500 border-slate-500/20",
    SUBMITTED: "bg-blue-500/10 text-blue-500 border-blue-500/20",
    WAITING_FOR_SYNC: "bg-purple-500/10 text-purple-500 border-purple-500/20",
    VERIFIED: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20",
    MISMATCH: "bg-orange-500/10 text-orange-500 border-orange-500/20",
    CORRECTION_REQUIRED:
      "bg-amber-500/10 text-amber-500 border-amber-500/20",
    REJECTED: "bg-red-500/10 text-red-500 border-red-500/20",
  };
