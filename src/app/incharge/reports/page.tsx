...
// ============================================================================
// HELPER FUNCTIONS & TYPES (UNCHANGED LOGIC)
// ============================================================================
...
export default async function ReportsPage(props: {
  ...
  const reports: CombinedReport[] = await getAllReports(searchParams);

  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[1400px] flex-col gap-4 px-4 py-8 pt-20 sm:gap-6 sm:px-5 sm:py-12 md:pt-24 lg:gap-6 lg:px-6">
      ...
      <header className="flex flex-col gap-4">
        <div className="flex flex-col items-start gap-1.5">
          ...
          <p className="max-w-3xl text-sm text-[var(--muted-foreground)]">
            Same employee reports are combined date-wise. If one employee submits or updates multiple country reports on the same day, it will appear as one combined row.
          </p>
        </div>

        {/* Status Tabs */}
        <div className="flex w-full overflow-x-auto pb-1 scrollbar-none sm:pb-0">
          <div className="flex min-w-max items-center gap-1 rounded-lg border border-[var(--border)] bg-[var(--card)] p-1 shadow-sm">
            {["ALL", "APPROVED", "PENDING", "REJECTED"].map((tab) => (
              <Link
                key={tab}
                href={buildStatusHref({
                ...
                className={`rounded-md px-3 py-1.5 text-xs font-semibold uppercase tracking-wider transition-all duration-200 sm:px-4 sm:py-2 ${
                  statusTab === tab
        ...
        <form className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className="relative flex-1 sm:flex-none">
              <CalendarDays className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted-foreground)]" />
              <input
                type="date"
                name="from"
                defaultValue={from}
                className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] py-2 pl-9 pr-3 text-sm font-medium text-[var(--foreground)] outline-none transition focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)] sm:w-[140px]"
              />
            </div>
            ...
            <div className="flex w-full gap-2 sm:w-auto">
              <button
                type="submit"
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2 sm:flex-none"
              >
              ...
              <Link
                href="?status=ALL"
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--background)] px-4 py-2 text-sm font-semibold text-[var(--foreground)] transition hover:bg-[var(--accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] sm:flex-none"
              >
                <RotateCcw className="h-4 w-4 text-[var(--muted-foreground)]" />
                Clear
              </Link>
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1000px] text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {reports.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-[var(--muted-foreground)] sm:py-16">
                ...
                reports.map((report) => {
                  ...
                  return (
                    <tr
                      key={report.rowId || `${report.userId}-${report.combinedDateKey}`}
                      className="group/row transition-colors hover:bg-[var(--background)]/50"
                    >
                      {/* Personnel & Date */}
                      ...
                      {/* Employee ID */}
                      <td className="px-4 py-3 sm:px-5 sm:py-3.5">
                        <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-0.5 font-mono text-[10px] font-semibold text-[var(--muted-foreground)] shadow-sm transition-colors group-hover/row:border-[var(--primary)]/30 group-hover/row:text-[var(--primary)]">
                      ...
                      <td className="px-4 py-3 sm:px-5 sm:py-3.5">
                        <div className="flex items-center gap-1.5 whitespace-nowrap text-xs font-semibold text-[var(--foreground)] sm:gap-2 sm:text-sm">
                          ...
                          <span className="truncate max-w-[120px]">{formatCountry(report)}</span>
                        </div>
                      </td>
                      ...
                      {/* Combined Count */}
                      <td className="px-4 py-3 sm:px-5 sm:py-3.5">
                        <span className="inline-flex items-center justify-center whitespace-nowrap rounded-md bg-[var(--background)] border border-[var(--border)] px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">
                      ...
                      {/* Status */}
                      <td className="px-4 py-3 sm:px-5 sm:py-3.5">
                        <span className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-md border px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider shadow-sm ${statusStyle.bg} ${statusStyle.text} ${statusStyle.border}`}>
                      ...
                      <td className="px-4 py-3 text-right sm:px-5 sm:py-3.5">
                        <div className="flex justify-end opacity-100 transition-opacity lg:opacity-0 lg:group-hover/row:opacity-100">
                          <ReportRowActions
                            reportId={report.latestReportId || report.id}
                            reportIds={report.reportIds}
                            currentStatus={report.status}
